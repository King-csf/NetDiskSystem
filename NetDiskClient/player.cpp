...
void Player::playAudio()
{
    ...
    if(!endFrame || !speedFrame)
    {
        qDebug() << "音频帧分配失败";
        if(endFrame) av_frame_free(&endFrame);
        if(speedFrame) av_frame_free(&speedFrame);
        return;
    }
        ...
        else
        {
            if(ret == AVERROR(EAGAIN))
            {
                ...
                if(frame)
                {
                    ...
                    ret = av_buffersrc_add_frame_flags(srcBuffer,speedFrame,AV_BUFFERSRC_FLAG_KEEP_REF);
                    if(ret < 0)
                    {
                        qDebug() << "向滤镜添加帧失败, error:" << ret;
                    }
...
void Player::initSoundBar()
{
    volume = 50;
    ui->sound_bar->setValue((int)volume);
}

void Player::initProgressBar()
{
    while(!isDemuxer);
...
void Player::on_start_btn_clicked()
{
    isPause = false;
    connect(timer,&QTimer::timeout,this,[&](){
        curSec += 1* speed;
...
void Player::on_next_btn_clicked()
{
...
void Player::on_last_btn_clicked()
{
    int value = curSec - JUMP_TIME;
    if(value < 0)
    {
...
//进度条跳转
void Player::on_progress_bar_sliderPressed()
{
...
void Player::on_speedComboBox_currentIndexChanged(int index)
{
...
void Player::on_sound_bar_sliderPressed()
{
    int curVolume = ui->sound_bar->value();
    isModVolume.store(true);
    volume.store((float)curVolume/volume);
}
